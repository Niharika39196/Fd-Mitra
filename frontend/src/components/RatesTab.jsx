...
const TENOR_LABELS = { '6M': '6 Months', '1Y': '1 Year', '2Y': '2 Years', '3Y': '3 Years' };
const SAFE_COLORS = {
  'Highest': '#16a34a',
  'Very High': '#0891b2',
  'High': '#f59e0b',
...
export default function RatesTab({ onAskQuestion }) {
  ...
  const handleAsk = (bank, tenor) => {
    const q = `${bank.bank} ${bank.rates[tenor]}% p.a. ${tenor} FD safe hai kya? Agar main ₹${principal.toLocaleString('en-IN')} lagaun toh kitna milega?`;
    onAskQuestion(q);
  };
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px' }}>
          Tap any rate to ask FD Mitra about it instantly
        </div>
      </div>
        ...
        {TENORS.map(t => (
          <button
            ...
            style={{
              ...
              background: selectedTenor === t ? 'linear-gradient(135deg, #0055cc, #00aaff)' : 'rgba(255,255,255,0.06)',
              color: selectedTenor === t ? '#fff' : 'rgba(255,255,255,0.5)',
              fontSize: '13px', cursor: 'pointer', fontWeight: selectedTenor === t ? '600' : '400',
              fontFamily: "'DM Sans', sans-serif",
              boxShadow: selectedTenor === t ? '0 4px 12px rgba(0,100,255,0.3)' : 'none',
              transition: 'all 0.2s'
            }}
          >{TENOR_LABELS[t]}</button>
        ))}
      </div>
        ...
        {types.map(t => (
          <button
            ...
            style={{
              ...
              border: filterType === t ? '1px solid rgba(0,180,255,0.5)' : '1px solid rgba(255,255,255,0.08)',
              background: filterType === t ? 'rgba(0,180,255,0.1)' : 'transparent',
              color: filterType === t ? '#00b4ff' : 'rgba(255,255,255,0.35)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1, minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '3px' }}>
                  ...
                  <span style={{
                    fontSize: '10px', padding: '1px 7px', borderRadius: '10px',
                    background: `${SAFE_COLORS[bank.safe]}22`, color: SAFE_COLORS[bank.safe]
                  }}>🛡 {bank.safe}</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
              <div style={{ textAlign: 'right' }}>
                <div style={{
                  ...
                  {bank.rates[selectedTenor]}%
                </div>
                <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)' }}>p.a.</div>
              </div>
              <button
                onClick={() => handleAsk(bank, selectedTenor)}
                style={{
      ...
      <div style={{
        ...
        <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px', marginBottom: '18px' }}>
          See exactly how much you will earn
        </div>

        {/* Principal input */}
        <div style={{ marginBottom: '14px' }}>
          <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '12px', marginBottom: '6px', fontWeight: '500' }}>
            Investment Amount
          </div>
        ...
        <div style={{ marginBottom: '14px' }}>
          ...
          <select
            value={calcBank.short}
            onChange={e => setCalcBank(FD_RATES.find(b => b.short === e.target.value))}
            style={{
          ...
          >
            {FD_RATES.map(b => (
              <option key={b.short} value={b.short} style={{ background: '#071428' }}>
                {b.bank} — {b.rates[calcTenor]}% ({calcTenor})
              </option>
            ))}
          </select>
        </div>
            ...
            {TENORS.map(t => (
              <button
                ...
                style={{
                  ...
                  background: calcTenor === t ? 'linear-gradient(135deg, #0055cc, #00aaff)' : 'rgba(255,255,255,0.06)',
                  color: calcTenor === t ? '#fff' : 'rgba(255,255,255,0.5)',
                  fontSize: '12px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif",
                  transition: 'all 0.2s', fontWeight: calcTenor === t ? '600' : '400'
                }}
              >{t}</button>
            ))}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
          <div style={{
            ...
            <div style={{ color: 'rgba(0,180,255,0.7)', fontSize: '11px', marginBottom: '6px', fontWeight: '600' }}>
              MATURITY AMOUNT
            </div>
          ...
          <div style={{
            ...
            <div style={{ color: 'rgba(0,229,204,0.7)', fontSize: '11px', marginBottom: '6px', fontWeight: '600' }}>
              INTEREST EARNED
            </div>
            <div style={{ color: '#00e5cc', fontSize: '20px', fontWeight: '800' }}>
              {formatINR(interest)}
            </div>
            <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: '11px', marginTop: '4px' }}>
              at {calcBank.rates[calcTenor]}% p.a.
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div>
            <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '11px', marginBottom: '3px' }}>
              vs Savings Account (3.5% p.a.)
            </div>
            <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '13px' }}>
              You would only earn {formatINR(savingsInterest)}
            </div>
          </div>
          <div style={{
            ...
            <div style={{ color: 'rgba(16,185,129,0.6)', fontSize: '10px' }}>extra with FD</div>
          </div>
        </div>

        {/* Ask button */}
        <button
          onClick={() => handleAsk(calcBank, calcTenor)}
          style={{
        ...
        >
          Ask FD Mitra about this →
        </button>
      </div>

      <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.15)', textAlign: 'center', paddingBottom: '16px' }}>
        * Rates are indicative. Verify with bank before investing. DICGC insures up to ₹5 lakh.
      </div>
    </div>
  );
}
